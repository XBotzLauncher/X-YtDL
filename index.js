const XTelegram = require("node-telegram-bot-api");
...
XBotz.onText(/\/ytmp4 (.+)/, async (msg, match) => {
  ...
  XBotz.sendMessage(chatId, "Download sedang berjalan!");
  try {
    const { result } = await savetube.download(url, "360");
    const caption = `*Information Video*: \n\nId: ${result.id}\nTitle: ${result.title}\nType: ${result.type}\nFormat: ${result.format}\nDuration: ${result.duration}\nQuality: ${result.quality}\nDownload: [Klik disini](${result.download})\n\n*Thanks for use me:D*`;
    const foto = result.thumbnail;
    XBotz.sendPhoto(chatId, foto, {
      caption,
      parse_mode: "Markdown"
    });
  } catch (error) {
    console.error("Error: " + error.message);
    XBotz.sendMessage("Error to download, because" + error.message);
  }
})

XBotz.onText(/\/ytmp3 (.+)/, async (msg, match) => {
  const chatId = msg.chat.id;
  const url = match[1];
  ...
  try {
    ...
    const caption = `*Information Audio*: \n\nId: ${result.id}\nTitle: ${result.title}\nType: ${result.type}\nFormat: ${result.format}\nDuration: ${result.duration}\nQuality: ${result.quality}\nDownload: [Klik disini](${result.download})\n\n*Thanks for use me:D*`;
    const audio = result.thumbnail;
    XBotz.sendPhoto(chatId, audio, {
      caption,
      parse_mode: "Markdown"
    });
  } catch (error) {
